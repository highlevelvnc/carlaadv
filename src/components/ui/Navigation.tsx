...
import type { Locale, Content } from '@/lib/content'
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="flex border border-white/10 overflow-hidden">
              {(['pt', 'en'] as Locale[]).map(l => (
                <button key={l} onClick={() => onLocale(l)}
          ...
          {/* Hamburger */}
          <button onClick={toggle} className="md:hidden flex flex-col gap-[5px] p-2" aria-label="Menu" aria-expanded={open}>
            <span className={`block w-[22px] h-px bg-glacier transition-all duration-300 ${open ? 'rotate-45 translate-y-[7px]' : ''}`} />
            <span className={`block w-[22px] h-px bg-glacier transition-all duration-300 ${open ? 'opacity-0' : ''}`} />
            <span className={`block w-[22px] h-px bg-glacier transition-all duration-300 ${open ? '-rotate-45 -translate-y-[7px]' : ''}`} />
          </button>
        </div>
      </motion.nav>
          ...
          >
            {links.map((l, i) => (
              <motion.a
                ...
                className="font-serif text-3xl text-glacier/60 hover:text-glacier font-light letter-spacing tracking-wide transition-colors"
              >
                {l.label}
              </motion.a>
            ))}
            <div className="flex gap-3 mt-4">
              {(['pt', 'en'] as Locale[]).map(l => (
                <button key={l} onClick={() => { onLocale(l); close() }}
